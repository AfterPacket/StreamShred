...
def linux_drop_page_cache_best_effort(fd: int, file_size: int) -> None:
    ...
    try:
        ...
        # Align to page size for best effect (not strictly required).
...
def capture_expected_from_chunk(
    ...
    """
    As we stream-write, record expected bytes for any sample ranges that overlap this chunk.
    expected[i] holds expected bytes for sample i.
    filled[i] is a bytearray mask (0/1) tracking which expected bytes were captured.
    """
        ...
        for p in range(1, passes + 1):
            ...
            while written < size:
                ...
                if verify and samples:
                    capture_expected_from_chunk(samples, expected, filled, data, written)
            ...
            if verify and samples:
                for m in filled:
                    if not all_filled(m):
                        raise RuntimeError("Internal verification capture failed (sample not fully captured).")
            ...
            print(f"[*] {APP_NAME}: pass {p}/{passes} complete" + (" (verified)" if verify else ""))
...
def wipe_file(
    ...
    *,
    ...
    print(f"[*] {APP_NAME}: size={size} bytes | passes={passes} | chunk={chunk_size} bytes")
    ...
    final_path = rename_truncate_unlink(file_path, rename_passes=rename_passes, keep=keep)
    if keep:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--pick", action="store_true", help="GUI file picker (if available)")
    ap.add_argument("--min-passes", type=int, default=3, help="Minimum passes (default 3)")
    ap.add_argument("--max-passes", type=int, default=7, help="Maximum passes (default 7)")
    ap.add_argument(
        "--no-randomize-pass-count",
        action="store_true",
        help="Use exactly min-passes instead of random in [min,max]",
    )
    ap.add_argument("--chunk", type=int, default=1024 * 1024, help="Chunk size bytes (default 1 MiB)")
    ap.add_argument("--verify", action="store_true", help="Enable read-back verification samples per pass")
    ap.add_argument("--verify-samples", type=int, default=6, help="Samples per pass when verifying (default 6)")
    ap.add_argument("--verify-len", type=int, default=64, help="Bytes per sample when verifying (default 64)")
    ap.add_argument("--rename-passes", type=int, default=2, help="Rename passes before delete (default 2)")
    ap.add_argument(
        "--drop-cache",
        action="store_true",
        help="Linux only: posix_fadvise(DONTNEED) after each pass (best-effort)",
    )
    ap.add_argument("--keep", action="store_true", help="Do not delete after shredding (testing)")
    ap.add_argument("--force", action="store_true", help="Skip confirmation prompt")
    args = ap.parse_args(argv)

    target: Optional[Path] = Path(args.file).expanduser() if args.file else None

    if args.pick or target is None:
        try:
            ...
            root = tk.Tk()
            root.withdraw()
            picked = filedialog.askopenfilename(title=f"Select a file to shred ({APP_NAME})")
            if not picked:
                print("[*] No file selected.")
                return 0
            target = Path(picked)
        except Exception as e:
            print(f"[!] GUI picker unavailable ({e}). Provide a file path.")
            return 2
